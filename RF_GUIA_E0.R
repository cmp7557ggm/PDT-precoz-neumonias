...
set.seed(123)
mod1<-rfsrc(Surv(alta_cruda,Muertos )~.,data=RF_sf,ntree = 500, nodesize = 5,mtry = 25,
            importance = T)
mod1

plot(gg_vimp(mod1))  

plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("ED0","Cancer","Hongo","Charlson_Index","NFR"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("Edad","recepcion","Chronic_Kidney_disease","Pseudomonas_a","Provisional"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("COVID","Ingreso_UCI"))
