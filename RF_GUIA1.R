...
df<-datarf_nzv
df$ED0<-neumonia$ED0
#df$diagetiol<-neumonia$diagetiol_cruda
df$cambiodat<-neumonia$cambiodat_cruda
...
set.seed(123)
mod1<-rfsrc(Surv(alta_cruda,Muertos )~.,data=df,ntree = 500, 
            importance = T)
mod1 

o <- tune(Surv(alta_cruda,Muertos)~.,data=df)
o$optimal
set.seed(123)
mod1<-rfsrc(Surv(alta_cruda,Muertos )~.,data=df,ntree = 500,nodesize = 6,mtry = 10, 
            importance = T)
mod1 
...
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("DER","Cancer","Hongo","Antibiotic","Periodo"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("Pre_post","recepcion","rapida_cruda","Charlson_Index","GRIPE"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("DER_hecho","Edad","Chronic_Kidney_disease","COVID","Smoking_history"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("E_coli","NFR","PCR","Neumococo","Staphilococcus_a"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("Ingreso_UCI","Hypertension","T_empirico","Haemophilus_I","Drinking_history"))
######
# QUITAMOS LAS VARIABLES PERIODO Y DIGNOSTICO RAPIDO Y REPETIMOS EL PROCESO

RF_sf<-dplyr::select(df,-c(Periodo,Pre_post,DER,PCR,DER_hecho))

save(RF_sf,file="/Users/carlosmartinperez/Desktop/NEUMONIAS/BUENA/novedades/RF_sf.RData")
...
set.seed(123)
mod1<-rfsrc(Surv(alta_cruda,Muertos )~.,data=RF_sf,ntree = 500,nodesize = 15,mtry = 25, 
            importance = T)
mod1 

plot(gg_vimp(mod1))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("ED0","cambiodat"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("Cancer","Hongo","Antibiotic"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("recepcion","rapida_cruda","Charlson_Index","GRIPE"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("Edad","Chronic_Kidney_disease","COVID","Smoking_history"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("E_coli","NFR","PCR","Neumococo","Staphilococcus_a"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("Ingreso_UCI","Hypertension","T_empirico","Haemophilus_I","Drinking_history"))
...
###################3
# Modelo con menor error :Hongo, antibiotic,recepcion, COVID
# error 0,316  

set.seed(123)
mod2<-rfsrc(Surv(alta_cruda,Muertos )~Hongo+Antibiotic+recepcion+COVID,data=df,ntree = 1000,nodesize = 15,mtry = 25, 
            importance = T,)
mod2 

plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("recepcion","Hongo","Antibiotic","COVID"))

###################3
# Modelo EXPLICATIVO :Hongo, antibiotic,recepcion, COVID+ Cancer + Charlson_Index, Edad
# error 0,316   

set.seed(123)
mod3<-rfsrc(Surv(alta_cruda,Muertos )~Hongo+Antibiotic+recepcion+COVID+Cancer+Charlson_Index+
              Edad+GRIPE+E_coli,data=df,ntree = 1000,nodesize = 15,mtry = 25, importance = T)
mod3 

plot.variable(mod3, partial=T,plots.per.page = 1, xvar.names = c("recepcion","Hongo","Antibiotic","COVID",
                                                                 "Cancer","Charlson_Index","Edad","GRIPE","E_coli"))
plot.variable(mod1, partial=T,plots.per.page = 1, xvar.names = c("Staphilococcus_a","E_coli","Pseudomonas_a",
                                                                 "Neumococo","Haemophilus_I"))
